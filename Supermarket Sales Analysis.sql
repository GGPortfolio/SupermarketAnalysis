...
ALTER TABLE 
  sales ALTER COLUMN rating TYPE NUMERIC(3, 1);
-- Changed precision to 3 to allow values up to 99.9
...
SELECT 
  ...
  AND '16:00:00' THEN 'Afternoon' ELSE 'Evening' END AS time_of_day 
FROM 
  sales;
--
ALTER TABLE 
...
--
SELECT 
  to_char(date, 'Day') as name_of_day 
FROM 
  sales;
ALTER TABLE 
  sales 
ADD 
  COLUMN name_of_day VARCHAR(20);
UPDATE 
  sales 
SET 
  name_of_day = (
...
WITH product_sales AS (
  ...
  WHERE 
    ...
    AND 3 -- Filter for January to March 2019
  GROUP BY 
    Product_line 
  ORDER BY 
    total_sales DESC
...
SELECT 
  ps.product_line, 
  CAST(
    ROUND(ps.total_sales, 2) AS money
  ) AS total_sales, 
  EXTRACT(
    MONTH 
    FROM 
...
FROM 
  product_sales AS ps 
  JOIN sales AS s ON ps.product_line = s.product_line 
WHERE 
  EXTRACT(
    YEAR 
    FROM 
  ...
  CONCAT(
    ROUND(
      COALESCE(
        ...
        --Using NULLIF to avoid errors in division.
        0
      ), 
      2
    ), 
...
ORDER BY 
  branch, 
  gender;
----2. What is the average spend per A)customer, B)customer type, and C)gender.Who is your best customer?
--A)
SELECT 
  ROUND(
    AVG(total), 
